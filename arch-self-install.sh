...
if systemd-detect-virt --quiet 2>/dev/null; then
    IS_VM=true
    VM_TYPE="$(systemd-detect-virt 2>/dev/null || echo "baremetal")"
    log_info "Environnement détecté : ${VM_TYPE}"
fi
...
log_info "CPU Cores détectés : ${CPU_CORES}"
...
partition_and_encrypt() {
    ...
    if [[ "$INSTALL_MODE" == "clean" ]]; then
        ...
        # ── Création de la partition ESP (512 MiB) avec alignement à 1 Mo ──
        log_info "Création de la partition ESP (512 MiB)..."
        sgdisk -n 1:0:+512M -t 1:ef00 -c 1:"EFI" --align=1 "${TARGET_DISK}"

        # ── Création de la partition racine (reste du disque) avec alignement à 1 Mo ──
        log_info "Création de la partition racine (reste du disque)..."
        sgdisk -n 2:0:0 -t 2:8309 -c 2:"CRYPTROOT" --align=1 "${TARGET_DISK}"

        partprobe "${TARGET_DISK}" 2>/dev/null || true
        sleep 2

        # Déterminer le nom de la partition selon le disque (nvme vs sda)
    ...
    # Vérifier que ROOT_PART est défini avant de procéder
    if [[ -z "$ROOT_PART" ]]; then
        die "La partition racine (ROOT_PART) n'est pas définie."
    fi
    ...
    log_info "Chiffrement de la partition racine avec LUKS..."
    cryptsetup luksFormat --type luks2 --cipher aes-xts-plain64 --key-size 512 --hash sha512 --pbkdf argon2id --pbkdf-memory 262144 --pbkdf-parallel 2 --pbkdf-force-iterations 4 --sector-size 4096 --batch-mode "${ROOT_PART}"

    log_ok "Chiffrement LUKS configuré."

    # ── Ouverture du volume LUKS ──
    log_info "Ouverture du volume chiffré..."
    cryptsetup open "${ROOT_PART}" cryptroot

    # Vérification que le volume est ouvert
    if [[ ! -b /dev/mapper/cryptroot ]]; then
        die "Impossible d'ouvrir le volume chiffré."
    fi
    log_ok "Volume chiffré ouvert : /dev/mapper/cryptroot"
}

    # ── Determine LUKS Argon2id params based on available RAM ── 
    ARGON_MEM=262144  # 4GB default
    ARGON_PARALLEL=${CPU_CORES}

    # Ajustement des paramètres LUKS Argon2id en fonction de la RAM
    if (( TOTAL_RAM_GB >= 128 )); then
        ARGON_MEM=8388608  # 8GB for 128GB+ RAM systems
    elif (( TOTAL_RAM_GB >= 64 )); then
        ARGON_MEM=4194304  # 4GB for 64GB+ RAM systems
    elif (( TOTAL_RAM_GB >= 32 )); then
        ARGON_MEM=2097152  # 2GB for 32GB+ RAM systems
    elif (( TOTAL_RAM_GB >= 16 )); then
        ARGON_MEM=1048576  # 1GB for 16GB RAM systems
    elif (( TOTAL_RAM_GB >= 8 )); then
        ARGON_MEM=524288   # 512MB for 8GB RAM systems
    elif (( TOTAL_RAM_GB >= 4 )); then
        ARGON_MEM=262144   # 256MB for 4GB RAM systems
    fi

# ── Assurer que ROOT_PART est définie ──
if [[ -z "$ROOT_PART" ]]; then
    die "La partition racine (ROOT_PART) n'est pas définie. Assurez-vous qu'elle soit assignée avant d'exécuter le chiffrement."
fi
