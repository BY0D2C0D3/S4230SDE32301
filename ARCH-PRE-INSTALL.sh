...
echo "💡 Utilisateur choisi : $USER_NAME"
...
# -----------------------------
if [[ "$INSTALL_MODE" == "clean" ]] && [[ "$INSTALL_DISK" != /dev/loop* ]]; then
    wipefs -a $INSTALL_DISK
else
    echo "💡 Ignoring wipefs on loop device $INSTALL_DISK"
fi
...
# Vérification cryptroot
if ! [ -e /dev/mapper/cryptroot ]; then
    echo "❌ Erreur : cryptroot non ouvert. Veuillez vérifier le mot de passe LUKS."
    exit 1
fi
...
# Vérification fstab
echo "💡 Vérification /mnt/etc/fstab :"
cat /mnt/etc/fstab
...
# Locale + clavier AZERTY + MX Keys Mini
...
# Systemd services pour snapshots et optimisations
...
# Hyprland optimisations CPU/GPU
...
echo "🎉 Installation v4.3 Ultimate Expert terminée !"
echo "💡 Vérifiez /dev/mapper/cryptroot et les subvolumes Btrfs avant reboot."
