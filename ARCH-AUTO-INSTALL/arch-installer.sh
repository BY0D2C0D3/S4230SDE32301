...
run_bootstrap_phase() {
    ...
    # .config/ is essential -- it holds all the Hyprland/quickshell/etc dotfiles
    # that imperative-dots/install.sh lays down at $HOME. Without it, the
    # dotfiles phase would fall back to a fresh clone from upstream (which
    # re-introduces the telemetry we stripped). Fail loudly NOW.
    if [[ ! -d "$DOTS_SRC/.config" ]]; then
        die "$DOTS_SRC/.config is missing. Verify your repo on GitHub includes \
the hidden directory (cd $SCRIPT_DIR && ls -la imperative-dots/). If it was \
not pushed, re-add it on the host with: 'git add -f imperative-dots/.config' \
then commit + push, then re-clone in this VM."
    fi
