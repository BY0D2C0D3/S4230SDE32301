...
run_bootstrap_phase() {
    ...
    # .config/ holds the Hyprland/quickshell/etc dotfiles. If absent, the bare
    # Arch install completes fine and boots normally; only the dotfiles phase
    # at first login will fail (user can re-run it after fixing the payload).
    # Soft warn instead of hard die so the install proceeds.
    if [[ ! -d "$DOTS_SRC/.config" ]]; then
        log_warn "${DOTS_SRC}/.config is missing -- the dotfiles phase will fail at first login."
        log_warn "Bare Arch install will still complete and boot normally."
        log_warn "To fix later: re-push the .config/ folder to your repo, copy it into ~/imperative-dots/ on the installed system, then re-run the dotfiles phase manually."
        sleep 3
    fi
    [[ -d "$DOTS_SRC/.local" ]] || log_warn "$DOTS_SRC/.local missing -- some dotfiles will be incomplete."
    ...
    log_ok "Mount layout:"
    findmnt --target /mnt | tee -a "$LOG_FILE" || true
    ...
    local PACSTRAP_PKGS=(
        ...
        # Guest agents for the supported hypervisors -- only the rock-solid
        # ones go in pacstrap (hyperv is moved to a best-effort post-install
        # step because if Arch ever retires it, pacstrap as a single
        # transaction would fail entirely).
        ...
        # - spice-vdagent : SPICE console clipboard + auto-resize
        # gtk3 = soft dep of open-vm-tools GUI helpers.
        open-vm-tools gtk3
        qemu-guest-agent spice-vdagent
        # Wireless + Bluetooth stacks. `wireless_tools` (iwconfig/iwlist legacy)
        # is also moved post-pacstrap as it's deprecated and at risk of removal.
        iw wpa_supplicant
        bluez bluez-utils
...
CUSTOM_UA_PLATFORM=$(printf '%q' "${CUSTOM_UA_PLATFORM}")
CUSTOM_UA_OSCPU=$(printf '%q' "${CUSTOM_UA_OSCPU}")
CUSTOM_UA_APPVERSION=$(printf '%q' "${CUSTOM_UA_APPVERSION}")
CUSTOM_UA_BUILDID=$(printf '%q' "${CUSTOM_UA_BUILDID}")
CUSTOM_ACCEPT_LANG=$(printf '%q' "${CUSTOM_ACCEPT_LANG}")
VM_TYPE="${VM_TYPE}"
EOF
...
# Best-effort install of optional packages that are at risk of being retired
# from official repos (or that don't ship for every kernel). Failures are
# tolerated -- they only impact niche use cases.
#  - hyperv          : Hyper-V Integration Services (only meaningful on Hyper-V)
#  - wireless_tools  : legacy iwconfig/iwlist (deprecated; only useful for
#                      old apps that still call iwconfig)
pacman -Sy --noconfirm --needed hyperv 2>/dev/null \
    && echo "[CHROOT] hyperv installed" \
    || echo "[CHROOT-WARN] hyperv unavailable; Hyper-V hv_*_daemon services may be missing." >&2
pacman -S  --noconfirm --needed wireless_tools 2>/dev/null \
    && echo "[CHROOT] wireless_tools installed" \
    || echo "[CHROOT-WARN] wireless_tools unavailable; legacy iwconfig won't be present (iw is fine)." >&2
...
# Take ownership of the ENTIRE home directory. /home/${USERNAME} was created
# by the parent script (cp -a) BEFORE useradd ran in this chroot, so useradd
# saw an existing dir, skipped skel copy, and left /home/${USERNAME} owned by
# root. Fix it recursively here so the user can write to ~/.cache and friends.
